...
# 3. Producing the file necessary for PGDSpider program
...
popnames <- as.character(
  c(
    ...
    "pop = Mainland", # 29, 139, 140, 162, 349, 350 ((including Luzon here because Luzon is part of the mainland cluster in the NeighborNet network)
    "pop = Malenge", # 163:174
...
# 4. Loading the results files for analysis
...
# 5. Creating results dataframe on which to base analyses

## 5a. Converting FST matrix to dataframe
...
## 5b. Making Geopgraphic distance matrix
...
## 5c. Combining the FST and Geographic dataframes
...
z <- pwd.df[pwd.df$fst.dist >= 0,] # removing 132 (New_Guinea:Mainland) and 148 (Normanby_Island:Mainland) which are both negative FST values

x <- lm(fst.dist ~ geo.dist, data = z) # creating model to take weights from
wt <- 1 / lm(abs(x$residuals) ~ x$fitted.values)$fitted.values^2 # weighting residuals by how large they are
rm(x) 

LM <- lm(fst.dist ~ geo.dist, data = z, weights = wt) # weighted residual model
...
shapiro.test(LM$residuals) # W = 0.99402, p-value = 0.2898, indicates normality of residuals

ncvTest(LM) # homoscedasticity test: Chisquare = 2.234525e-07, Df = 1, p = 0.99962, H0 of constant variance is not rejected. Supported by flat-ish slope in Scale-location plot (plot(testLM))
...
> Call:
lm(formula = fst.dist ~ geo.dist, data = x, weights = wt)
