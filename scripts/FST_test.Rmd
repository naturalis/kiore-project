...
z <- pwd.df[pwd.df$fst.dist >= 0,] # removing 132 (New_Guinea:Mainland) and 
...
y <- under.fv[under.fv$fst.dist < under.fv$fv - 0.2,] # keeping only fst.dist values far below fitted values
y <- y[order(y$islands.combo, decreasing = FALSE),] # sorting alphabetically
kable(y)
```

```{r network, eval = FALSE}
# in progress, not evaluated 
library(GGally)
library(network)
library(sna)
foo <- pwd[1:5,1:5]
foo <- as.network(foo, directed = FALSE, names.eval = "weights")
ggnet2(foo, edge.size = "weights")


island.km <- read.csv("./data/Raw_data/island_size_data.csv", header = TRUE)
island.km <- island.km[,c(2, 4)]
foo %v% "area_km2" <- c(island.km[1:5,2])
```
...
# this plot is best seen in the viewer rather than in the knitted pdf. Has also been saved as a separate file.
```
