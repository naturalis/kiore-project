...
z <- dist.summary # making copy
z[z == 0] <- NA # turning zeros to NAs
z <- na.omit(z) # removing rows with NAs
```
...
testLM <- lm(gen.dist ~ geo.dist, data = z) # model
summary(testLM) # model results

plot(gen.dist ~ geo.dist, data = z)
...
par(mfrow = c(1, 1))
residualPlots(testLM) # curved therefore some non-linearity possible, Tukey test p-value is significant, indicating non-linearity
...
ks.test(testLM$residuals, 'pnorm') # D = 0.4324, p-value < 2.2e-16, non-normal
hist(testLM$residuals, breaks = 50) # clear positive skew

ncvTest(testLM) # homoscedasticity test: Chisquare = 947.4107, Df = 1, p = < 2.22e-16, H0 of constant variance is rejected
...
GLM <- glm(gen.dist ~ geo.dist, data = z, family = Gamma(link = "identity")) # Gamma distrubution family used for right-skewed data. 
...
|:----:|:----:|:----:|:----:|:----:|
|-1.7511|-0.5344|-0.2203|0.1489|2.9980|  

> Coefficients:

| |Estimate|Std. Error|t value|p-value|
|:----:|:----:|:----:|:----:|:----:|
|(Intercept)|9.593e-02|6.969e-04|137.7|<2e-16 ***|
|geo.dist|1.988e-05|1.620e-07|122.7|<2e-16 ***|

> Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

> (Dispersion parameter for Gamma family taken to be 0.4959804)

> Null deviance: 30851  on 65255  degrees of freedom  
Residual deviance: 23001  on 65254  degrees of freedom  
AIC: -118035  
