...
```{r plots with labels}
names(dist.summary)
ggplot(data = dist.summary, aes(x = geo.dist, y = gen.dist)) + geom_point(shape = 1, colour = "grey") + geom_smooth(method = "lm") + geom_quantile(quantiles = c(0.05,0.95)) + geom_text(aes(label = ifelse(gen.dist > 1.12, as.character(islands.combo),'')), hjust=0, vjust=0)
