...
# 2. Prepping df for HWE Analysis
...
## 2b. Producing the file necessary for PGDSpider program
...
popnames <- as.character(
  c(
    ...
    "pop = Mainland", # 29, 146, 147, 169, 358, 359 (including Luzon here because Luzon is part of the mainland cluster in the NeighborNet network)
    "pop = Malenge", # 170:181
...
# 3. HWE Analysis and removal

## 3a. Creating loop for reading the HWE files
```{r reading HWE files}
getwd()
...
## 3b. Checking the HWE P-values

```{r making hwe results df}
objectnames <- as.vector(ls()) # should be islands only, otherwise remove extras from vector
objectnames
# If necessary:
objectnames <- objectnames[-c(3, 27, 28)] # removing non-island objects, may not be the same numbers
...
## 3c. Running Holm's Sequential Bonferroni test to adjust p-values
...
## 3d. Examining significant hwe p-values
...
### Significant adjusted p-values per Locus
...
### Number of island populations with a significant adjusted p-value at a particular locus
...
# 4. Removing samples/loci with issues identified in HWE and Structure analyses
```{r specimen removal}
getwd()
halfclean <- read.csv("./data/RStudio/ratsSNPs_halfclean.csv")

# need to remove Kamaka_009
# not removing Rimatuu_19 and Rimatuu_20 because they are right next to Reiono and could have swam.
...
# 5. Double checking for monomorphic columns (SNP loci)
...
# 6. Summary: specimens per Island after full data clean-up
