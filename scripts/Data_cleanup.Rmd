...
```{r SNP column separation, eval = FALSE}
x2 <- x # making a copy
...
Running a Holm's Sequential Bonferroni test to adjust p-values
```{r eval = FALSE}
nrow(hwe.all) # 2622

p.value.adjusted <- c(p.adjust(hwe.all$P.value, method = "holm")) # do I not need to insert the threshold??
hwe.all$p.value.adjusted <- p.value.adjusted

rm(p.value.adjusted)
```
General holm's formula:
target alpha level / no. of tests - rank no. of pair (by degree of signif.) + 1

Examining significant hwe p-values
```{r eval = FALSE}
hwe.signif <- hwe.all[which(hwe.all$p.value.adjusted <= 0.05),]
hwe.signif[,c(1,8,9)]
count(hwe.signif$Locus) # 2 at locus 41 (Kaikura and Reiono), rest are singles
count(hwe.signif$islandpop) # concerning that 17 of 23 are Kayangel
```
...
remove <- c("Kamaka_008", "Rimatuu_19", "Rimatuu_20", "Kayangel11", "Kayangel13", "Kayangel15", "Kayangel17", "Kayangel19", "Kayangel21") # names of specimens to remove (each name should have exactly 10 characters)
