...
```{r count per island 1, eval = FALSE}
count(data$island.1)
```
...
```{r invariant column search, eval = FALSE}
ncol(x) #333
...
```{r column deletion 1, eval = FALSE}
...
```{r reading HWE files, eval = FALSE}
getwd()
...
```{r making hwe results df, eval = FALSE}
...
```{r adjusted p-value summary, eval = FALSE}
...
```{r invariant column check, eval = FALSE}
ncol(clean) #298
monocols <- integer() # empty vector for the for loop
for (i in 17:298) {
...
```{r possible column deletion 2, eval = FALSE}
...
```{r count per island 2, eval = FALSE}
