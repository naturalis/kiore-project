...
## Dataset loading
```{r eval = FALSE}
data <- read.csv("./data/ratsSNPs_halfclean.csv") # loading the data table

z <- data # making a copy of the data
colnames(z)
z <- z[,-c(2:16)] # removing unnecessary columns for this analysis
```

## Heterozygosity experimenting
```{r Setting up data, eval = FALSE}
dim(z) # 379 rows 283 columns
str(z)
unique(unlist(z[,17:283])) # checking what SNP combinations are present
...
```{r homozygous totals per specimen, eval = FALSE}
o.freq <- vector()
e.freq <- vector()
na.freq <- vector()
for (i in 1:379) {
...
df <- data.frame(z$island, o.freq, e.freq, na.freq) # making a df with the freq totals
...
```{r homozygous total per island, eval = FALSE}
data <- data[order(data$island, decreasing = FALSE),] # ordering df alphabetically by island
unique(data$island)
unique(data$island.1)
colnames(data)
data[,c(1,8)][data$island.1 == "Mainland"] # doesn't work
...
for (i in 1:length(shortpopnames)) {
  y <- as.vector(grep(shortpopnames[i], z[,1], ignore.case = TRUE, value = FALSE))
  assign(paste(shortpopnames[i]), z[y,])
}
rm(y, i)
