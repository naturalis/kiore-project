...
per.island <- read.csv("./results/RStudio_Heterozygosity/heterozygosity_testing_results_table.csv", header = TRUE)
```
...
par(mfrow = c(1, 1))
residualPlots(testLM) # curved therefore some non-linearity possible, although Tukey test p-value is non-significant
...
par(mfrow = c(1, 1))
residualPlots(LM) # curved therefore some non-linearity possible, although Tukey test p-value 0.5348 (H0 not rejected therefore linearity), and the spread of points looks okay
...
par(mfrow = c(1, 1))
residualPlots(testLM2) # curved therefore non-linearity possible, however Tukey test p-value 0.5124. Spread of points doesn't look good, there's lots of bunching 

qqPlot(testLM2$residuals, line = "quartiles") # normal dist, outlier point 19
shapiro.test(testLM2$residuals) # W = 0.89572, p-value = 0.01482, indicates non-normality of residuals if alpha is 0.05
